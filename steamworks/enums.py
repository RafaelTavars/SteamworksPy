...
class EP2PSend(Enum):
    ...
    k_EP2PSendUnreliable = 0  # For data that doesn't need to arrive reliably, or has no value after it's missed.
    k_EP2PSendReliable = (
        1  # Data must arrive reliably.  Will resend until it is acknowledged.
    )
    k_EP2PSendUnreliableNoDelay = 2  # Unreliable, but tries to send immediately.
    k_EP2PSendReliableNoDelay = 3  # Reliable, but tries to send immediately.


class ELobbyType(Enum):
    """ELobbyType"""

    k_ELobbyTypePrivate = 0  # Not joinable, only invitee can join
    k_ELobbyTypeFriendsOnly = 1  # Joinable by friends and invitees
    k_ELobbyTypePublic = 2  # Joinable by anyone, visible in lobby list
    k_ELobbyTypeInvisible = 3  # Not joinable, and not visible in lobby list, only invitees can join, preferred for matchmaking
    k_ELobbyTypeFriendsOfFriends = 4  # Joinable by friends of friends
